...
public class FragmentHospitals extends Fragment {
    ...
    public FragmentHospitals(int color) {
        this.color = color;
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        FirebaseRecyclerOptions<Model_Hospitals> options =
                new FirebaseRecyclerOptions.Builder<Model_Hospitals>()
                        .setQuery(query, Model_Hospitals.class)
                        .build();

        adapter = new FirebaseRecyclerAdapter<Model_Hospitals, ViewHolderPlaces>(options) {
            @Override
            public ViewHolderPlaces onCreateViewHolder(ViewGroup parent, int viewType) {
                // Create a new instance of the ViewHolder, in this case we are using a custom
                // layout called R.layout.message for each item
                View view = LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.layout_place1, parent, false);

                return new ViewHolderPlaces(view);
            }

            @Override
            protected void onBindViewHolder(final ViewHolderPlaces holder, int position, final Model_Hospitals model) {
                ...
                holder.setItemClickListener(new ItemClickListener() {
                    ...
                    public void onClick(View view, int position, boolean isLongClick) {
                        Intent intent=new Intent(getActivity(),DetailHospital.class);
    ...
    private void addOrRemoveToSaved(String id, boolean isAdd, Model_Hospitals model) {
