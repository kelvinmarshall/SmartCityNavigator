...
public class DetailM_Churches extends AppCompatActivity {
    private ImageView image_M_church,image_select;
    private TextView desc,text_select;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detail_m__churches);
        ...
        image_M_church=findViewById(R.id.img_m_church);
        ...
        service_start=findViewById(R.id.start);
        service_end=findViewById(R.id.end);
        ...
        if(getIntent() !=null)
            m_churchId=getIntent().getStringExtra("id");
        if(!m_churchId.isEmpty())
        {
            load_info();
            retrieveCommnets();
        }
        //add saved
        if (Utils.savedRepository.isSaved(Integer.parseInt(m_churchId))==1)
    ...
    private void retrieveCommnets() {
        Query query = FirebaseDatabase.getInstance()
                .getReference()
                .child("Parks").child(m_churchId).child("comments")
                .limitToLast(50);
    ...
    private void postComment() {
        ...
        DatabaseReference comments=FirebaseDatabase.getInstance().getReference()
                .child("Parks").child(m_churchId).child("comments").push();
        ...
        comments.setValue(model_comments).addOnSuccessListener(new OnSuccessListener<Void>() {
            ...
            public void onSuccess(Void aVoid) {
                Toast.makeText(DetailM_Churches.this, "Comment added successfully", Toast.LENGTH_SHORT).show();
            ...
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(DetailM_Churches.this, "Comment post failed", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void load_info() {
        databaseReference=database.getReference().child("M_church").child(m_churchId);
        databaseReference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                model_mChurch=dataSnapshot.getValue(Model_MChurch.class);
                if (dataSnapshot.exists()){
                    toolbar.setTitle(model_mChurch.getName());
                    details.setText(model_mChurch.getDetails());
                    name.setText(model_mChurch.getName());
                    opendays.setText(model_mChurch.getOpendays());
                    openingtime.setText(model_mChurch.getOpeningtime());
                    location.setText(model_mChurch.getLocation());
                    service_start.setText(model_mChurch.getService_start());
                    service_end.setText(model_mChurch.getService_end());


                    Glide.with(getApplicationContext()).load(model_mChurch.getImage_url()).into(image_M_church);

                    directions.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Intent direction=new Intent(DetailM_Churches.this,Directions.class);
                            direction.putExtra("location",model_mChurch.getLocation());
                            direction.putExtra("lat",model_mChurch.getLat());
                            direction.putExtra("long",model_mChurch.getLong());
                            direction.putExtra("name",model_mChurch.getName());
                            direction.putExtra("image",model_mChurch.getImage_url());
                    ...
                    show_map.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Intent map=new Intent(DetailM_Churches.this,Maps.class);
                            map.putExtra("location",model_mChurch.getLocation());
                            map.putExtra("lat",model_mChurch.getLat());
                            map.putExtra("long",model_mChurch.getLong());
                            map.putExtra("name",model_mChurch.getName());
                            map.putExtra("image",model_mChurch.getImage_url());
                            startActivity(map);
                        }
                    });
                    btnsave.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Toast.makeText(DetailM_Churches.this, m_churchId, Toast.LENGTH_SHORT).show();
                            if (Utils.savedRepository.isSaved(Integer.parseInt(m_churchId))!=1)
                            {
                                addOrRemoveToSaved(m_churchId,true,model_mChurch);
                                btnsave.setImageResource(R.drawable.favorite);
                            }
                            else
                            {
                                addOrRemoveToSaved(m_churchId,false,model_mChurch);
                                btnsave.setImageResource(R.drawable.favorite_border);
                            }
                        }
                    });
    ...
    private void addOrRemoveToSaved(String parkId, boolean isAdd, Model_MChurch model_parks) {
